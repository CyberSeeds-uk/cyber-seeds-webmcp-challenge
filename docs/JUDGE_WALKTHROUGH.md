...
5. Record the `requestId` returned by the current refused invocation. Treat this exact value as request-scoped authority context for this run; do not reuse a historical request ID.
6. Physically activate **Approve this action** in the visible page. Approval must occur in the page, not through a Site Tool.
7. Check approval state, then retry `account-mfa` using the exact `requestId` returned by step 4. Confirm successful execution and `RESOLVED` state.
8. Replay the exact same target and exact same `requestId`. Confirm that no second consequence occurs and the resolved action remains non-replayable.
